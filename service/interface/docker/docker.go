...
func NewCli() (*client.Client, error) {
	...
	for _, context := range contextMap {
		if context.Name == curContext {
			contextEndpoint := context.Endpoints["docker"].(dockerContext.EndpointMetaBase)
			currContextHost = contextEndpoint.Host
			break
		}
	}
	return client.NewClientWithOpts(client.WithHost(currContextHost))
}
